...
@endphp
    @if($jenisretur == "pembelian")
        <form class="form-horizontal" role="form" action="{{ route('retur.update', ['id' => $trx_id]) }}" enctype="multipart/form-data" method="POST">
        {{ method_field('PUT') }}
    @elseif($jenisretur == "penjualan")
        <form class="form-horizontal" role="form" action="{{ route('retur.updatepj', ['id' => $trx_id]) }}" enctype="multipart/form-data" method="POST">
        {{ method_field('PUT') }}
    @endif
