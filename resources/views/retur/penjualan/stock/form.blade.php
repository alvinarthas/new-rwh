        ...
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-10">
                        <input type="text" class="form-control" disabled value="{{$trx->customer()->first()->apname}}">
                    </div>
                </div>
                ...
                <table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @foreach ($delivered as $deliv)
                            <tr>
                                <td>
                                    <a href="javascript:;" onclick="getDetail('{{ $deliv->id_jurnal }}')" class="btn btn-purple btn-rounded waves-effect w-xs waves-danger m-b-5">
                                        {{$deliv->id_jurnal}}
                                    </a>
                                </td>
                                <td>{{$deliv->date}}</td>
                                <td>{{$deliv->creator()->first()->name}}</td>
                                <td>
                                    @if(array_search("RJDOD",$page))
                                        <a href="javascript:;" onclick="deleteDeliv('{{$deliv->id_jurnal}}')" class="btn btn-danger btn-rounded waves-effect w-xs waves-danger m-b-5">Delete</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
            ...
            <form class="form-horizontal" role="form" action="{{ route('returjual.updatedelivery', ['id' => $trx->id]) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <input type="hidden" name="trx_id" value="{{$trx->id}}">
                <div class="card-box table-responsive">
                    <div class="row">
                        <h4 class="m-t-0 header-title">Create Retur Delivery Order Details</h4>
                        <div class="col-12">
                            <div class="p-20">
                                <table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                    <thead>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-10">
                                        <div class="input-group">
                                            <input type="text" class="form-control" parsley-trigger="change" required placeholder="yyyy/mm/dd" name="delivery_date" id="delivery_date" data-date-format='yyyy-mm-dd' autocomplete="off">
                            ...
                            @if (array_search("RJDOC",$page))
                                <div class="form-group text-right m-b-0">
                                    <button class="btn btn-custom btn-rounded waves-effect waves-light w-md m-b-5">Create Retur Delivery Order</button>
                                </div>
                            @endif
                        </div>
                    </div>
...
<script>
    ...
    function deleteDeliv(id){
        ...
        swal({
            ...
            $.ajax({
                url: "{{route('returjual.destroydelivery')}}",
                type: 'GET',
                data: {
                    jurnal_id: id,
                    _token: token,
